...
| Atlas | `gpt-image-2-1k` | 支持 | 1 张源图 | 仅宽高比 | 正整数；受服务限制 | 取决于模型 | 取决于模型 |
| 自定义 OpenAI 兼容服务 | 按配置顺序 | 按声明能力 | 按声明能力 | 取决于端点 | 取决于端点 | 取决于端点 | 取决于端点 |

不受支持的统一参数会明确报错，不会被静默丢弃。
...
- GPT Image 2 请求需要上游计费所需的显式 `size`；未指定时使用 `1024x1024`。不接受 `1K`、`2K`、`4K` 别名；可传入上游接受的明确像素尺寸，但实际输出以服务响应为准。
