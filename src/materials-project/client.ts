...
export const PROPERTY_FIELDS: Record<MaterialProperty, readonly string[]> = {
  ...
  bandstructure: ["material_id", "bandstructure", "band_gap", "cbm", "vbm", "efermi", "is_gap_direct", "structure", "origins", "warnings"],
  dos: ["material_id", "dos", "structure", "origins", "warnings"],
  ...
  phonon: ["material_id", "phonon_bandstructure", "phonon_dos", "structure", "origins", "warnings"],
  ...
  xas: ["material_id", "spectrum", "spectrum_name", "absorbing_element", "spectrum_type", "edge", "structure", "origins", "warnings"],
  insertion_electrodes: ["material_id", "battery_id", "working_ion", "average_voltage", "capacity_grav", "capacity_vol", "energy_grav", "energy_vol", "task_ids", "warnings"],
  ...
  eos: ["material_id", "eos", "energies", "volumes", "warnings"],
  ...
  substrates: ["sub_form", "sub_id", "film_orient", "area", "energy", "film_id", "norients", "orient"],
  alloys: ["alloy_pair", "pair_id", "alloy_system", "alloy_id"],
  similarity: ["material_id", "sim", "feature_vector", "method", "origins", "warnings"],
  synthesis: ["material_id", "synthesis_type", "synthesis", "keywords", "origins", "warnings"],
};

function validatePropertyFields(property: MaterialProperty, fields: string[]): void {
  // Keep the shared tool schema compatible with route records whose canonical
  // identity is film_id or pair_id; the capability matrix still documents the
  // route-specific fields and limitations.
  const allowed = new Set([...PROPERTY_FIELDS[property], "material_id"]);
  const invalid = fields.filter((fieldName) => !allowed.has(fieldName));
...
function normalizeRow(row: Record<string, unknown>, provenance: Provenance, requestedFields: string[]): MaterialRecord {
  const id = row.material_id ?? row.materialId;
  if (typeof id !== "string") throw new MaterialsProjectError("SCHEMA_MISMATCH", "response row has no material_id");
...
export class MaterialsProjectClient {
  ...
  constructor(config: MaterialsProjectConfig = {}, fetcher: MaterialsFetch = fetch as unknown as MaterialsFetch) {
    ...
    this.apiKey = config.apiKey ?? (config.credentialEnv ? process.env[config.credentialEnv] : process.env.MP_API_KEY);
  ...
  private async request(path: string, query: URLSearchParams, signal?: AbortSignal): Promise<{ payload: unknown; url: string }> {
    if (!this.apiKey) throw new MaterialsProjectError("AUTH_REQUIRED", "set MP_API_KEY or configure a Materials Project credentialEnv");
  ...
  async get(options: MaterialsGetOptions, signal?: AbortSignal): Promise<MaterialsGetResult> {
    ...
    const pageSize = Math.min(1000, maxResults);
    for (const property of properties) {
      const path = PROPERTY_SUFFIX[property];
      ...
      const idKey = property === "tasks" ? "task_ids" : "material_ids";
      const ids = property === "tasks" ? taskIds : materialIds;
      if (property === "tasks" && !ids.length) throw new MaterialsProjectError("SCHEMA_MISMATCH", "tasks property requires taskIds; task IDs are not material IDs");
      if (property !== "tasks" && !ids.length) throw new MaterialsProjectError("SCHEMA_MISMATCH", `${property} property requires materialIds`);
      if (MATERIAL_ID_UNSUPPORTED_PROPERTIES.has(property) && !options.fields) throw new MaterialsProjectError("UNSUPPORTED_CAPABILITY", `${property} retrieval requires route-specific filters that materials_get does not expose`);
      ...
      while (page < maxPages && requestsMade < maxPages && propertyRecords < maxResults) {
        ...
        for (const row of parsed.rows) {
          const next = normalizeRow(row, provenance, fields);
          const previous = recordsById.get(next.materialId);
          if (!previous) {
            recordsById.set(next.materialId, next);
          } else {
