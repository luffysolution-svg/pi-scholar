...
export class ZoteroClient {
  ...
  private async request(pathname: string, init: RequestInit = {}, signal?: AbortSignal): Promise<Response> {
    ...
    catch (e) { if (signal?.aborted) throw signal.reason ?? new Error("Zotero request cancelled"); throw new Error(`Zotero Desktop unavailable at ${this.baseUrl}. Start Zotero, confirm port 23119 is reachable, and enable “Allow other applications on this computer to communicate with Zotero” in Zotero settings. Details: ${e instanceof Error ? e.message : String(e)}`, { cause:e }); }
  ...
  async searchItems(q = "", opts: { collectionKey?: string; itemType?: string; limit?: number; sort?: "dateAdded"|"dateModified"|"title"|"creator"|"date"; direction?: "asc"|"desc" } = {}, signal?: AbortSignal): Promise<ListResult<ZoteroRaw>> { const term=q.trim();const browsing=!term||term==="*";const query={q:browsing?undefined:term,qmode:browsing?undefined:"everything",itemType:opts.itemType,sort:opts.sort??(browsing?"dateModified":undefined),direction:opts.direction??(browsing?"desc":undefined)};if (opts.collectionKey) { validateKey(opts.collectionKey,"Collection key"); const requested=Math.min(Math.max(opts.limit ?? 50,1),this.maxItems);const all = await this.list<ZoteroRaw>(`/collections/${opts.collectionKey}/items`, query, this.maxItems, signal); const matched = all.items.filter(x => !x.data.deleted && !x.data.parentItem && Array.isArray(x.data.collections) && x.data.collections.includes(opts.collectionKey!));const items=matched.slice(0,requested); return { ...all, items, total: matched.length, truncated:all.truncated||matched.length>items.length }; } return this.list("/items/top", query, opts.limit ?? 50, signal); }
