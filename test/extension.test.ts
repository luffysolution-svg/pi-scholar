...
test("Pi entry point registers integrated Ai4Scholar and local tools exactly once",()=>{
  const h=harness();const names=h.tools.map(t=>t.name);for(const name of ["ai4scholar_search","ai4scholar_paper","ai4scholar_cite","ai4scholar_citation_candidates","zotero_collections","zotero_search","zotero_item","pi_scholar_parse","pi_scholar_image_models","pi_scholar_image_generate","pi_scholar_image_edit","pi_scholar_image_service"])assert.equal(names.filter(n=>n===name).length,1,`${name} registration`);
  assert.ok(!names.some(n=>n.includes("mcp")));
